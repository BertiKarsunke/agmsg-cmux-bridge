...
- `scripts/actas-claim.sh`は、`actas` claimが成功した後、現在のpaneに登録を更新します。
...
Pane対応は`scripts/lib/pane.sh`と`scripts/pane.sh`にあります。バックエンドは自動検出されます。tmuxは`$TMUX`と`tmux`が利用可能なときに`$TMUX_PANE`を使います。cmuxは`cmux identify --json`の`caller.surface_ref`値を使い、必要に応じて`$CMUX_SURFACE_ID`をfallbackとして使います。
